...
def main():    
    ...
    print(autoencoder)
    summary(autoencoder.encoder,(1,32,32)) #3
    ...
    # show images 
    imshow(torchvision.utils.make_grid(images.reshape(32,1,32,32)), 'cifar10_results',text_ =' '.join('%5s' % autoencoder.classes[labels[j]] for j in range(4))) #3
    # print labels
    ...
    latent_dim,ver = "Gray2ClolarizationResizeVgg16", "100"  #####save condition
    dataiter = iter(autoencoder.testloader)
    ...
    encode_img,_ = pretrained_model.encoder(images[0:32].to('cpu').reshape(32,1,32,32)) #3
