...
def main():    
    ...
    print(autoencoder)
    #summary(autoencoder.encoder,(1,32*2,32*2)) #3
    #summary(autoencoder,(1,32,32)) #3
    ...
    # show images 
    imshow(torchvision.utils.make_grid(images.reshape(32,1,32*2,32*2)), 'cifar10_results',text_ =' '.join('%5s' % autoencoder.classes[labels[j]] for j in range(4))) #3
    # print labels
    ...
    latent_dim,ver = "Gray2ClolarizationResizeVgg16x2G01x2", "100"  #####save condition
    dataiter = iter(autoencoder.testloader)
    images0,images, images1, labels = dataiter.next()
    # show images
    imshow(torchvision.utils.make_grid(images.reshape(32,1,32*2,32*2)),'original_images_cifar10_{}_{}'.format(latent_dim,ver),text_ =' '.join('%5s' % autoencoder.classes[labels[j]] for j in range(4)))
    # show images0
